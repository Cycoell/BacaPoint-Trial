...
<?php while ($row = mysqli_fetch_assoc($result)) { ?>
  <form action="../config/update_buku.php" method="POST" class="flex bg-gray-100 p-4 rounded-md border border-blue-400 shadow items-center space-x-4" id="form-<?= $row['id'] ?>">
    ...
    <div class="flex-1 space-y-2 text-sm">
      ...
      <div class="flex items-center gap-2">
        ...
        <input type="text" name="judul" value="<?= $row['judul'] ?>" 
          class="flex-1 border rounded px-2 py-1 bg-transparent" 
          readonly data-original="<?= htmlspecialchars($row['judul']) ?>">
      </div>
      <div class="flex items-center gap-2">
        <label class="w-24 font-medium">Tahun:</label>
        <input type="text" name="tahun" value="<?= $row['tahun'] ?>" 
          class="flex-1 border rounded px-2 py-1 bg-transparent" 
          readonly data-original="<?= htmlspecialchars($row['tahun']) ?>">
      </div>
      <div class="flex items-center gap-2">
        <label class="w-24 font-medium">Author:</label>
        <input type="text" name="author" value="<?= $row['author'] ?>" 
          class="flex-1 border rounded px-2 py-1 bg-transparent" 
          readonly data-original="<?= htmlspecialchars($row['author']) ?>">
      </div>
      <div class="flex items-center gap-2">
        <label class="w-24 font-medium">Genre:</label>
        <input type="text" name="genre" value="<?= $row['genre'] ?>" 
          class="flex-1 border rounded px-2 py-1 bg-transparent" 
          readonly data-original="<?= htmlspecialchars($row['genre']) ?>">
      </div>
    ...
    <div class="flex flex-col space-y-2" id="actions-<?= $row['id'] ?>">
      ...
      <button onclick="hapusBuku(<?= $row['id'] ?>)" type="button" class="bg-red-500 hover:bg-red-600 text-white px-4 py-1 rounded">Hapus</button>
    </div>
